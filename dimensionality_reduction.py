def demensionality_reduction(train_frame,test_frame,n_comp=320,random_seed=420,decompositions_to_run=['PCA','TSVD','ICA','GRP','SRP'],valid_frame=None,frame_type='h2o',test_does_have_y=False):
    """
        ...
        for i in xrange(1,iterations):
            ...
            if 'TSVD' in decompositions_to_run:
                ...
                if valid_frame:
                    ...
                    tsvd_results_valid = tsvd.fit_transform(valid_df.drop(["y"], axis=1))
                    if test_does_have_y:
            ...
            if 'PCA' in decompositions_to_run:
                ...
                pca_results_train = pca.fit_transform(train_df.drop(["y"], axis=1))
                if valid_frame:
                    pca2 = PCA(n_components=n_comp, random_state=random_seed)
                    pca_results_valid = pca2.fit_transform(valid_df.drop(["y"], axis=1))
                    if test_does_have_y:
            ...
            if 'ICA' in decompositions_to_run:
                ica = TruncatedSVD(n_components=n_comp, random_state=random_seed)
                ica_results_train = ica.fit_transform(train_df.drop(["y"], axis=1))
                if valid_frame:
                    ica2 = FastICA(n_components=n_comp, random_state=random_seed)
                    ica_results_valid = ica2.fit_transform(valid_df.drop(["y"], axis=1))
                    if test_does_have_y:
            ...
            if 'GRP' in decompositions_to_run:
                ...
                if valid_frame:
                    ...
                    grp_results_valid = grp2.fit_transform(valid_df.drop(["y"], axis=1))
                    if test_does_have_y:
            ...
            if 'SRP' in decompositions_to_run:
                ...
                if valid_frame:
                    ...
                    srp_results_valid = srp2.fit_transform(valid_df.drop(["y"], axis=1))
                    if test_does_have_y:
            ...
            for i in range(1, n_comp + 1):
                if 'PCA' in decompositions_to_run:
                    ...
                    if valid_df:
                        valid_df['pca_' + str(i)] = pca_results_train[:, i - 1]
                ...
                if 'SRP' in decompositions_to_run:
                    ...
                    test_df['srp_' + str(i)] = srp_results_train[:, i - 1]
